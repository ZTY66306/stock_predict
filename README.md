...
| --- | --- |
| `R` | 重跑当前页面(重新执行脚本) |
| `C` | 弹 "Manage cache" 菜单,可清缓存 |
| `?` | 显示快捷键帮助 |
| `Ctrl+C`(终端) | 关闭 streamlit 服务 |
...
下面按页面顺序,每个说清:**输入什么 / 点哪里 / 看到什么**。

#### 🏠 1. 首页(自动出现,不用点)

启动 dashboard 后第一眼看到的就是它:
- 顶部两个绿条:`✅ baostock 已登录` + `✅ akshare 已装(资金流/涨跌停可用)`
- 8 个快速入口卡片(每个带一句话简介)
- 模块清单表格(18 个模块一行一行)
- "关于本看板"折叠区

**典型操作:** 啥也不点,直接左侧栏选要去的功能。

#### 📈 2. K 线 + 指标

```
┌─────────────────────────────────────────────────┐
│ [股票: sh.600000            ]  [频率: d▾]  [复权: 前复权▾] │
│ [时间区间: 1y ▾]                                   │
│ ──────── 技术指标 ─────────                        │
...
│ [图:价格 + MA + MACD / RSI]                       │
│ ──────── K 线形态识别 ─────────                    │
│ ☑ morning_star ☑ engulfing_bullish ...            │
│ [🔍 识别形态]                                      │
└─────────────────────────────────────────────────┘
```

- 输入:股票代码/中文名 + 频率(d/w/m/分钟) + 复权方式 + 时间区间
- 勾选要加的指标(默认勾了 5 个常用)
- 点 **📊 加指标 + 画图** → 上面出价格+MA+BOLL 副图,下面出 MACD 或 RSI
- 勾选要识别的形态(可多选) → 点 **🔍 识别形态** → 出命中日期表
- 展开 "📋 看带指标的完整数据" / "下载 CSV"

#### 🔍 3. 选股

**上半部:模板选股**
- 选模板(如 `macd_golden` / `low_pe_high_turnover`)→ 选日期 → 选回看天数 → 选最多选几只
- 点 **🚀 跑选股** → 出命中的股票表(代码/收盘/PE 等)
- **下载 CSV** 直接存

**下半部:自定义条件**
- 多选内置条件(MA/MACD/KDJ/RSI/BOLL/量能/形态)→ 自动 AND
- 展开 "加量化条件" 选 PE/PB/涨跌幅 区间
- 点 **🎯 跑自定义选股** → 出表

#### 🎯 4. 回测

- 输入股票 + 选 8 套策略之一 + 时间区间
- 展开 **⚙️ 高级参数** 改:初始资金 / 手续费 / 滑点 / 止损 / 止盈 / 策略参数 JSON
- 点 **🚀 跑回测** → 顶部 5 个 metric 卡(总收益/年化/夏普/最大回撤/胜率)
- 下面两图:权益曲线 + 回撤
- 展开看完整指标表 + 交易明细 + 下载 CSV

#### 🕸️ 5. 网格复盘

- 输入股票(ETF 走 T+0 自动,股票走 T+1) + 模式(等比/等差)
- 网格区间:二选一
  - **手动**:填 lower/upper 价格
  - **自动**:填 lower_pct/upper_pct(0.85 / 1.15)+ lookback
- 网格数 + 每格股数 + 底仓 + 交易制度(auto/强制 T+0/强制 T+1)
- 展开 **⚙️ 风控** 改止损止盈 + 初始资金
- 点 **🚀 跑网格回测** → 5 个 metric 卡 + 价格+网格线+买卖点 + 资金曲线+持仓副图
- 展开看网格效率 + 交易明细 + 下载 CSV

#### 🧩 6. 多策略融合 + 稳健性(双 Tab)

**Tab 1: 多策略融合**
- 选股票 + 投票模式(weighted / majority / veto)
- 多选要融合的策略(默认 4 套)
- weighted 模式下可以给每套策略一个权重
- majority/veto 模式下要选最少同意数
- 点 **🚀 跑融合** → 4 个 metric + 权益曲线

**Tab 2: 滚动稳健性**
- 选股票 + 策略 + 窗口大小(默认 252 交易日)+ 步长(默认 63)
- 改策略参数 JSON(留空用默认)
- 点 **🚀 跑稳健性** → summary 表 + **4 个稳健性评分卡**(盈利 fold 占比 / 夏普稳定性 / 回撤温和度 / 总评分)
- 下面出各 fold 明细 + 下载 CSV

#### 💰 7. 智能定投

- 选股票(建议 ETF/宽基)+ 时间区间(默认 3 年)
- 每期投入(默认 2000 元)+ 频率(月/双周/周)
- 勾 **跑 4 策略对比**(默认勾上)就一键对比 lump_sum / pure / dip_buy / smart
- 不勾选 → 单策略模式,选具体策略
- 展开 **⚙️ 智能加仓参数** 改均线窗口(smart 用)/ 跌幅阈值(dip_buy 用)
- 点 **🚀 跑 4 策略对比** → 4 行对比表 + 4 策略市值曲线对比图

#### 📡 8. 资金流 / 涨跌停(需 akshare)

**Tab 1: 个股资金流**
- 输股票 → 自动拉近 60 个交易日 → 出表 + 下载 CSV

**Tab 2: 北向资金**
- 点 **🔄 刷新** → 拉沪股通/深股通/港股通当日数据

**Tab 3: 当日涨停股池**
- 选日期 → 点 🔄 刷新 → 出涨停股明细 + 下载 CSV

**Tab 4: 题材/连板**
- 选日期 → 出行业涨停排行 + 概念涨停排行 + **4 个市场情绪 metric**(涨停数/跌停数/炸板率/强弱打分)

#### 📊 9. 实盘模拟

- 输股票代码(逗号分隔)+ 选共用策略 + 初始资金 + 单次用 % 资金
- 选运行日期(默认今天)
- 填状态文件路径(默认 `./paper_state.json`)+ 可选 webhook URL
- 点 **🚀 跑一次模拟** → 4 个 metric(总权益/盈亏比/持仓数/信号数)
- 下面分别展示:**今日信号**(signal≠0 的) / **当前持仓**(shares>0 的)
- 展开看运行日志 + 下载状态文件 JSON
- 跑多次后底部自动出 "历史运行" 区(从状态文件读)
